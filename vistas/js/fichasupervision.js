...
function obtenerIDultimaFicha() {
	$.post("../controladores/fichasupervision.php?op=obtenerIDultimaFicha", {}, function (data, status) {
		data1 = JSON.parse(data);
		dataResp = data1.aaData[0];

		let nroFicha = parseInt(dataResp[0]) + 1;
		$("#idFicha").html(nroFicha);
	});
}

function agregarEventosIniciales() {
	var btnNuevo = document.getElementById("btnNuevo");

	btnNuevo.addEventListener("click", function () {
		obtenerIDultimaFicha();
		limpiar();
	});
}
init();