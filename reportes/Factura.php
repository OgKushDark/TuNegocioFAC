...
function addSociete( $nom, $adresse,$logo,$ext_logo )
{
	...
	$this->Image($logo , 6 ,6, 22 , 22 , $ext_logo);
...
function AddImage($logo2,$contador){

	if($contador==-1){

		$this->Image($logo2 , 180 ,62, 9 , 9);

	}else{
		$x=150;
		$y=15*($contador)+62;
		$this->Image($logo2 , $x ,$y, 9 , 9);
	}
...
// Client address
function addClientAdresse( $cliente,$domicilio,$num_documento)
{
	$r1     = $this->w - 201;
	$r2     = $r1 + 68;
	$y1     = 40;
	...
	$this->SetXY( $r1+18, $y1-3);
	$this->SetFont( "Arial", "", 10);
	$this->MultiCell( 152, 4, $cliente);
	$this->SetXY( $r1, $y1+2);
	$this->SetFont( "Arial", "", 10);
	$this->MultiCell( 152, 4, $domicilio);
	$this->SetXY( $r1, $y1+7);
...
// Client address
function addClientAdresse2( $observaciones, $formaPago)
{
	$r1     = $this->w - 201;
	$y1     = 56;
	$this->SetXY( $r1, $y1+4);
	$this->SetFont( "Arial", "B", 10);
	$this->MultiCell( 150, 4, $observaciones);

	$this->SetXY( $r1+34, $y1);
	$this->SetFont( "Arial", "", 10);
	$this->MultiCell( 150, 4, $formaPago);
}

// Client address
function addClientAdresse3( $formaPago)
{
	$r1     = $this->w - 201;
	$y1     = 56;

	$this->SetXY( $r1+34, $y1+5);
	$this->SetFont( "Arial", "", 10);
	$this->MultiCell( 150, 4, $formaPago);
}

// Client address
function addClientAdresse4( $formaPago)
{
	$r1     = $this->w - 201;
	$y1     = 56;
...
function addCadreEurosFrancs($impuesto)
{
	...
	$this->Cell(20,4, "DESCUENTO", 0, 0, "C");

	$this->SetXY( $r1, $y1+8.65 );
	$this->Cell(20,4, $impuesto, 0, 0, "C");

	$this->SetXY( $r1, $y1+11.95 );
...
//                  "qte"     => qte,
//                  "tva"     => code_tva );
function addTVAs( $igv, $subtotal ,$total,$moneda )
{
	$this->SetFont('Arial','',8);
	...
	$this->SetXY( $re, $y1+8.5 );
	$this->Cell( 17,4, $moneda.sprintf("%0.2F", ($subtotal-$total)), '', '', 'R');

	$this->SetXY( $re, $y1+11.5 );
